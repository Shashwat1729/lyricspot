...
(async () => {
  ...
  await sleep(12000);
  let txt = await snap();
  console.log("KNOWN song=" + /Never Gonna/i.test(txt) + " ts=" + /\d+:\d\d/.test(txt) + " badge=" + /Demo/.test(txt));
  await page.goto("https://shashwat1729.github.io/lyricspot/", { waitUntil: "networkidle0", timeout: 45000 });
  await sleep(1500);
  await toLyrics();
  ...
  await sleep(12000);
  txt = await snap();
  console.log("UNKNOWN song=" + /Yellow Submarine/i.test(txt) + " ts=" + /\d+:\d\d/.test(txt) + " err=" + /without a backend/i.test(txt) + " fake=" + /Demo Song/.test(txt));
  await browser.close();
  console.log("LOOP2_RETRY_DONE");
})();