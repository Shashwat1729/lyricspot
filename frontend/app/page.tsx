...
import type { ApiResponse, ProgressEvent } from '@/lib/api';
...
export default function Home() {
  ...
  const handleLoadingChange = (isLoading: boolean, progressEvent?: ProgressEvent | string | null) => {
    setLoading(isLoading);
    if (progressEvent && typeof progressEvent === 'object') {
      setProgress(progressEvent as ProgressEvent);
    } else {
      setProgress(null);
    }
  };
              ...
              <>
                ...
                <div className="flex justify-center mb-8">
                  <div className="relative inline-flex items-center rounded-full bg-[#111] border border-white/[0.06] p-1">
                    <button
                      onClick={() => setMode('mic')}
                      className={`relative z-10 flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-colors ${
                        mode === 'mic' ? 'text-green-400' : 'text-gray-400 hover:text-white'
                      }`}
                    >
                      {mode === 'mic' && (
                        <motion.span
                          layoutId="mode-toggle-indicator"
                          className="absolute inset-0 -z-10 rounded-full border border-green-500/30 bg-green-500/20"
                    ...
                    <button
                      ...
                      className={`relative z-10 flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-colors ${
                        mode === 'text' ? 'text-green-400' : 'text-gray-400 hover:text-white'
                      }`}
                    >
                      {mode === 'text' && (
                        <motion.span
